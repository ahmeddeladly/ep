...
def kl_mvn(m0, S0, m1, S1, sum_log_diag_cho_S0=None):
    ...
    dm = m1 - m0
    KL_div = (
        0.5 * (np.trace(cho_solve(choS1, S0)) + dm.dot(cho_solve(choS1, dm)) - len(m0))
        - sum_log_diag_cho_S0
        + np.sum(np.log(np.diag(choS1[0])))
    )
    return KL_div


def compare_plot(a, b, a_err=None, b_err=None, a_label=None, b_label=None, ax=None):
...
def plot_results(model_name, model_id=None, dist_id=None):
        ...
        res_d = [
            (
                ...
                (
                    res_d_file["var_" + par]
                    if par != "phi"
                    else np.diag(res_d_file["cov_" + par])
                ),
            )
            for par in pnames
        ]
    ...
    res_f_file = np.load(os.path.join(RES_PATH, "res_f_{}.npz".format(file_ending)))
    ...
    res_f = [
        (
            ...
            (
                ...
                if par != "phi"
                else np.diag(res_f_file["cov_" + par + "_full"])
            ),
        )
        for par in pnames
    ]
    ...
    for i in range(niter):
        KL_divs[i] = kl_mvn(
            m_phi_full, cov_phi_full, m_phi_i[i], cov_phi_i[i], sum_log_diag_cho_S0
        )
    ...
    if mix:
        axs[0].plot(np.arange(niter + 1), np.vstack((m_phi_i, res_d[0][0])))
        axs[1].plot(
            np.arange(niter + 1),
            np.sqrt(np.vstack((np.diagonal(cov_phi_i, axis1=1, axis2=2), res_d[0][1]))),
        )
    else:
        axs[0].plot(np.arange(niter), m_phi_i)
        axs[1].plot(np.arange(niter), np.sqrt(np.diagonal(cov_phi_i, axis1=1, axis2=2)))
    ...
    if mix:
        ...
        fig.suptitle("phi")
        # Mean
        compare_plot(m_phi_full, m_phi_i[-1], a_label="full", b_label="dep", ax=axs[0])
        axs[0].set_title("mean")
        # Var
        compare_plot(
            np.diag(cov_phi_full),
            np.diag(cov_phi_i[-1]),
            a_label="full",
            b_label="dep",
            ax=axs[1],
