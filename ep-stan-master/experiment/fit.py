...
def main(model_name, conf, ret_master=False):
    ...
    if conf.method == "both" or conf.method == "distributed" or ret_master:
        ...
        print("Run distributed EP algorithm for {} iterations.".format(conf.iter))
        m_phi_i, cov_phi_i, info = dep_master.run(conf.iter, save_last_fits=conf.mix)
        if info:
            ...
            raise RuntimeError("Dep algorithm failed with error code: {}".format(info))
    ...
    if conf.method == "both" or conf.method == "full":
        ...
        with suppress_stdout():
            time_full = timer()
            fit = stan_model.sampling(data=data, seed=seed, **conf.mc_full_opt)
        ...
        print("    max Rhat: {:.4}".format(np.max(fit.summary()["summary"][:-1, -1])))
            ...
            if ih is None:
                ...
                for k in range(K):
                    ...
                    if ih == 0:
                        ...
                        pmap.append(
                            tuple(
                                slice(i, i + Ns[k]) if i2 == ih else slice(None)
        ...
        for pi in range(len(phiers)):
            ...
            if ih is None:
                ...
                for k in range(K):
                    ...
                    # dimension's slice(None) can be left out
                    pmap.append(
                        tuple(k if i2 == ih else slice(None) for i2 in range(ih + 1))
                    )
                pmaps.append(pmap)
            ...
            if ih is None:
                ...
                for j in range(J):
                    for _ in range(Ns[j]):
                        ...
                        pmap.append(
                            tuple(
                                j if i2 == ih else slice(None) for i2 in range(ih + 1)
                            )
                        )
                        i += 1
                pmaps.append(pmap)
...
CONF_HELP = dict(
    ...
    prec_estim=(
        "estimate method for tilted distribution precision matrix, "
...
for conf in CONFS:
    if conf.startswith("mc_"):
        CONF_HELP[conf] = CONF_HELP[conf] + ", default ({} {} {} {})".format(
            *[CONF_DEFAULT[conf][k] for k in ["chains", "iter", "warmup", "thin"]]
        )
    else:
        CONF_HELP[conf] = CONF_HELP[conf] + ", default {}".format(CONF_DEFAULT[conf])
